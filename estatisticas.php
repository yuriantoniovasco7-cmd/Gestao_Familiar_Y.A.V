<?php
    ...
    $sql_etaria = "SELECT 
        SUM(CASE WHEN TIMESTAMPDIFF(YEAR, data_nascimento, CURDATE()) < 15 THEN 1 ELSE 0 END) AS criancas,
         ...
         SUM(CASE WHEN TIMESTAMPDIFF(YEAR, data_nascimento, CURDATE()) >= 65 THEN 1 ELSE 0 END) AS idosos,SUM(CASE WHEN estado_saude = 'Vivo' THEN 1 ELSE 0 END) as vivos,
        SUM(CASE WHEN estado_saude = 'Falecido' THEN 1 ELSE 0 END) as falecidos
    FROM pessoas";
...
<head>
    ...
    body { 
        ...
        .card { border: 1px solid #070707; padding: 15px; margin-bottom: 20px; border-radius: 5px; }
